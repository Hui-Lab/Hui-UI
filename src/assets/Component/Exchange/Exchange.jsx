...
const Exchange = () => {
	...
	const getToken = (id) => {
		if (activeChain.chainID === 59140) return LineaTokenList[id];
		if (activeChain.chainID === 1442) return PolygonZKEVMTokenList[id];
	};
	...
	const getTokenList = () => {
		if (activeChain.chainID === 59140) return LineaTokenList.slice(0, 4);
		if (activeChain.chainID === 1442) return PolygonZKEVMTokenList.slice(0, 4);
	};
	...
	const handleExchange = async () => {
		if (tokenIn.symbol === "HUI") {
			const result = await writeContract(config, {
				...
				args: [tokenOut.address, parseUnits(tokenOutAmount, tokenOut.decimals)],
			});
		} else {
			...
			if (allowance.toString() < parseUnits(tokenInAmount, tokenIn.decimals)) {
				const approveResult = await writeContract(config, {
					abi: erc20Abi,
					address: tokenIn.address,
					functionName: "approve",
					args: [
						getHUIProviderAddress(),
						parseUnits(tokenInAmount, tokenIn.decimals),
					],
				});
			}
			const result = await writeContract(config, {
				...
				<div className="ExchangeInput">
					<div className="ExchangeInputLabel">
						<div
							...
							className={`tokenSymbolContainer ${
								tokenIn.symbol === "HUI" ? "" : "Changeable"
							}`}
						>
							{" "}
				...
				<div className="ExchangeOutput">
					<div className="ExchangeOutputLabel">
						<div
							...
							className={`tokenSymbolContainer ${
								tokenOut.symbol === "HUI" ? "" : "Changeable"
							}`}
						>
